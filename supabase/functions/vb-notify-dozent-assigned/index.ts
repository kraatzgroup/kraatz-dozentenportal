console.log('🚀 vb-notify-dozent-assigned edge function loaded');
...
interface DozentAssignedRequest {
  caseId: string;
...
Deno.serve(async (req) => {
  ...
  console.log('🚀 vb-notify-dozent-assigned function started');
  ...
  try {
    const { caseId, targetDozentId } = await req.json() as DozentAssignedRequest;
    ...
    // Security: only an admin may trigger this (manual assignment happens in
    // the admin dashboard). Recipient address is NEVER taken from the request
    // – it is loaded from the profiles table via targetDozentId.
    ...
    if (!callerRoles.includes('admin')) {
      console.error(`❌ [${requestId}] Forbidden: caller ${authData.user.id} is not an admin`);
      return new Response(
        JSON.stringify({ error: 'Forbidden: only admins can trigger this' }),
    ...
    const adminName = callerProfile?.full_name || 'Ein Administrator';
    ...
    const { error: notifError } = await supabaseAdmin
      ...
      .insert({
        ...
        title: 'Fall zugewiesen',
        message: `${adminName} hat Ihnen Klausur #${caseData.case_study_number ?? '?'} (${areaParts || 'Unbekannt'}) zugewiesen.`,
        type: 'info',
    ...
    const emailHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff;">
        ...
        <div style="padding: 30px 20px; background-color: white;">
          <h2 style="color: #333; margin: 0 0 20px 0; font-size: 20px;">Fall wurde Ihnen zugewiesen (Videoklausurenkorrektur)</h2>
          ...
          <p style="color: #555; font-size: 16px; line-height: 1.6; margin-bottom: 25px;">
            ${adminName} hat Ihnen den folgenden Fall zugewiesen. Die Zuständigkeit liegt nun bei Ihnen.
          </p>
    ...
    if (mailgunApiKey) {
      ...
      const emailSubject = `Fall zugewiesen: Klausur #${caseData.case_study_number ?? '?'} (${caseData.legal_area})`;
      ...
      try {
        ...
        if (mailgunResponse.ok) {
          ...
          console.log(`✅ [${requestId}] Assignment email sent to ${dozent.email}`);
          await logNotification({
            edgeFunction: 'vb-notify-dozent-assigned',
            supabaseAdmin,
    ...
    console.error(`❌ [${requestId}] Error in vb-notify-dozent-assigned:`, error);
    return new Response(
