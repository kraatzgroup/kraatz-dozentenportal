/*
  # Monthly Document Check System (Updated - Excluding Aktive Teilnehmer)

  1. Functions
    - `is_fifth_of_month()` - Check if today is the 5th
    - `get_previous_month_info()` - Get previous month details
    - `check_dozent_documents()` - Check specific dozent's documents
    - `get_monthly_document_status()` - Get status for all dozenten
    - `trigger_monthly_document_check()` - Main execution function
    - `get_next_deadline()` - Calculate next deadline
    - `is_past_deadline()` - Check if past current deadline

  2. Document Categories
    - Only checks: Rechnungen, Tätigkeitsbericht
    - Excludes: Aktive Teilnehmer (not subject to monthly deadline)

  3. Integration
    - Works with check-monthly-documents edge function
...
-- Function to check documents for a specific dozent (excluding Aktive Teilnehmer)
...
BEGIN
  ...
  WITH required_categories AS (
    -- Only check Rechnungen and Tätigkeitsbericht for monthly deadline
    SELECT unnest(ARRAY['Rechnungen', 'Tätigkeitsbericht']) AS category_name
  ),
  document_counts AS (
    SELECT 
    ...
    WHERE f.user_id = dozent_id 
      AND f.name IN ('Rechnungen', 'Tätigkeitsbericht')
    GROUP BY f.name
  )
  SELECT 
    rc.category_name,
...
-- Function to get monthly document status for all dozenten (excluding Aktive Teilnehmer)
...
BEGIN
  ...
  SELECT 
    ...
    ds.full_name,
    ds.email,
    2 AS total_categories, -- Only 2 required categories (Rechnungen, Tätigkeitsbericht)
    (
    ...
    (
      SELECT COUNT(*) = 2 -- Complete when both categories have documents
      FROM jsonb_array_elements(ds.details) AS elem
      WHERE (elem->>'has_documents')::BOOLEAN = true
    ) AS is_complete,
    ds.details
...
BEGIN
  ...
  IF NOT (is_deadline OR force_check) THEN
    RETURN QUERY SELECT 
      ...
      jsonb_build_object(
        ...
        'note', 'Only Rechnungen and Tätigkeitsbericht are checked for monthly deadline'
      );
    RETURN;
  END IF;
  ...
  -- Log the execution
  RAISE LOG 'Monthly document check executed on % for %/% - Total: %, Complete: %, Incomplete: % (Rechnungen & Tätigkeitsbericht only)', 
  ...
  RETURN QUERY SELECT 
    ...
    jsonb_build_object(
      ...
      'checked_categories', ARRAY['Rechnungen', 'Tätigkeitsbericht'],
      'excluded_categories', ARRAY['Aktive Teilnehmer'],
      'note', 'Aktive Teilnehmer is not subject to monthly deadline requirements',
...
BEGIN
  ...
  RAISE NOTICE '- Checked categories: Rechnungen, Tätigkeitsbericht';
  RAISE NOTICE '- Excluded categories: Aktive Teilnehmer (not subject to monthly deadline)';
