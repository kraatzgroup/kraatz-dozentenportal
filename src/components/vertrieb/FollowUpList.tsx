...
import { Phone, Clock, AlertCircle, Check, Plus, Edit2, Trash2 } from 'lucide-react';
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"
                placeholder="z.B. Interesse an Paket, Rückfragen..."
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
          ...
          pendingFollowUps.map((followUp) => (
            <div
              ...
              className={`p-4 hover:bg-gray-50 transition ${
                isOverdue(followUp.follow_up_date) ? 'bg-red-50' : 
                isToday(followUp.follow_up_date) ? 'bg-yellow-50' : ''
              }`}
            >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center space-x-2">
                    <h3 className="font-medium text-gray-900 truncate">
                      {followUp.teilnehmer_name}
                    </h3>
                    <span className={`px-2 py-0.5 text-xs rounded-full ${getPriorityColor(followUp.priority)}`}>
                    ...
                    {isToday(followUp.follow_up_date) && (
                      <span className="text-xs text-yellow-600 font-medium">Heute</span>
                    )}
                  </div>
                  <div className="mt-1 flex items-center text-sm text-gray-500 space-x-3">
                    <span className="flex items-center">
                      ...
                      {followUp.follow_up_time && ` um ${followUp.follow_up_time}`}
                    </span>
                    {followUp.teilnehmer_phone && (
                      <a href={`tel:${followUp.teilnehmer_phone}`} className="text-primary hover:underline">
                        {followUp.teilnehmer_phone}
                      </a>
                    )}
                  </div>
                  {followUp.reason && (
                    <p className="mt-1 text-sm text-gray-600">{followUp.reason}</p>
                  )}
                </div>
