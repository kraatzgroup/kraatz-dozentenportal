...
export function DozentenRechnungen() {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const filePath = `rechnungen/${id}/${fileName}`;
    ...
    const now = new Date();
    await supabase.from('dozent_rechnungen').insert({
      dozent_id: id,
      file_name: file.name,
      file_url: publicUrl,
      file_type: file.type,
  ...
  const deleteRechnung = async (rechnung: Rechnung) => {
    if (!confirm('Rechnung wirklich löschen?')) return;
    await supabase.from('dozent_rechnungen').delete().eq('id', rechnung.id);
    fetchRechnungen();
  };
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {rechnungen.map(r => (
            <div key={r.id} className="flex items-center justify-between bg-white rounded-lg shadow-sm p-4">
              <div className="flex items-center gap-3">
                <FileText className="h-8 w-8 text-primary" />
                <div>
                  <p className="font-medium text-gray-900">{r.file_name}</p>
                  <p className="text-sm text-gray-500">{r.month} {r.year}</p>
                </div>
              </div>
